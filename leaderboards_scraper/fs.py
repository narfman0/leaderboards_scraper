...
PLAYERS_JSON_PATH = f"{DATA_PATH}/players/parsed_players.json"
CATEGORIES_JSON_PATH = f"{DATA_PATH}/categories/categories.json"


def does_raw_run_exist(category_id, page_number):
    return os.path.exists(f"{DATA_PATH}/runs/raw_{category_id}_{page_number}.json")


def does_raw_player_exist(player_id):
    return os.path.exists(f"{DATA_PATH}/players/raw_{player_id}.json")


def load_raw_player_json(player_id):
    with open(f"{DATA_PATH}/players/raw_{player_id}.json") as file:
...
def load_raw_run_json(category_id, page_number):
    ...
    with open(f"{DATA_PATH}/runs/raw_{category_id}_{page_number}.json") as file:
        result = json.loads(file.read())
...
def store_parsed_category_runs_page(category_id, page_number, runs):
    with open(f"{DATA_PATH}/runs/parsed_{category_id}_{page_number}.json", "w") as file:
...
def store_parsed_runs(category_id, runs):
    with open(f"{DATA_PATH}/runs/parsed_{category_id}_runs.json", "w") as file:
        file.write(json.dumps(runs, default=pydantic_encoder))
...
def store_raw_category_runs_page(category_id, page_number, runs_page):
    # store in database to compare if runs are updated? maybe later
    with open(f"{DATA_PATH}/runs/raw_{category_id}_{page_number}.json", "w") as file:
...
def store_raw_player(player_id, player_data):
    with open(f"{DATA_PATH}/players/raw_{player_id}.json", "w") as file:
...
def store_category_leaderboard(category_id, markdown):
    with open(f"{DATA_PATH}/leaderboards/{category_id}.md", "w") as file:
        file.write(markdown)
